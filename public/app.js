// app.js - single file handles login/register, dashboard and admin UI for memory server
const API = '';
...
// Simple page routing based on HTML files: if index.html show forms, if dashboard/admin use connected logic
...
function connectSocket(){
  ...
  socket.on('banned', ()=>{ alert('You were banned'); localStorage.removeItem('token'); location.href='index.html'; });
...
function renderWallet(user){
  ...
  w.innerHTML=''; const b = user.balances||{}; let total=0;
  for(const k of Object.keys(b)){ const v=b[k]||0; const el=document.createElement('div'); el.textContent = `${k}: ${v}`; w.appendChild(el); if(k==='EUR') total+=v; else { const pair = `${k}/EUR`; const rate = latestPrices[pair]||0; total += v * rate; } }
  $('total-eur').textContent = fmtEUR(total);
}

// chart
function updateChart(prices){
  for(const p in prices){
    const arr = chartData[p] || []; arr.push(Number(prices[p])); if(arr.length>chartMaxPoints) arr.shift(); chartData[p]=arr;
  }
  drawChart();
}
function drawChart(){
  ...
  const w=c.width, h=c.height, len=data.length; const max=Math.max(...data), min=Math.min(...data);
...
function renderPrice(prices){
  const p = prices && prices[$('pair-select')?.value] ? prices[$('pair-select').value] : 0;
  const el = $('price'); if(el) el.textContent = fmtEUR(p);
}
...
function initDashboard(){
  ...
  connectSocket();
  $('logout').addEventListener('click', ()=>{ localStorage.removeItem('token'); location.href='index.html'; });
  $('chat-send').addEventListener('click', ()=>{ const text=$('chat-input').value.trim(); if(!text) return; socket.emit('send_chat',{ token: localStorage.getItem('token'), text }); $('chat-input').value=''; });
  $('buy').addEventListener('click', ()=>{ const amt=Number($('trade-amount').value)||0; socket.emit('trade',{ token: localStorage.getItem('token'), pair: $('pair-select').value, type:'buy', amountBase: amt }); });
  $('sell').addEventListener('click', ()=>{ const amt=Number($('trade-amount').value)||0; socket.emit('trade',{ token: localStorage.getItem('token'), pair: $('pair-select').value, type:'sell', amountBase: amt }); });
  $('pair-select').addEventListener('change', ()=> drawChart());
  // fetch initial wallet via admin users endpoint fallback
...
function initAdmin(){
  if(!localStorage.getItem('token')) return location.href='index.html';
  connectSocket();
  $('admin-logout').addEventListener('click', ()=>{ localStorage.removeItem('token'); location.href='index.html'; });
  $('admin-chat-send').addEventListener('click', ()=>{ const text=$('admin-chat-input').value.trim(); const user = prompt('Reply to username:'); if(!user||!text) return; socket.emit('admin_reply',{ token: localStorage.getItem('token'), username: user, text }); $('admin-chat-input').value=''; fetchUsers(); });
  $('export-all').addEventListener('click', ()=>{ window.open('/api/transactions/export', '_blank'); });
  fetchUsers();
  setInterval(fetchUsers, 2000);
}

async function fetchUsers(){ const res = await fetch('/api/admin/users', { headers:{ Authorization:'Bearer '+localStorage.getItem('token') } }); if(!res.ok) return; const arr = await res.json(); const tbody = $('users-table').querySelector('tbody'); tbody.innerHTML=''; const list = $('admin-userlist'); if(list) list.innerHTML=''; arr.forEach(u=>{ const tr=document.createElement('tr'); tr.innerHTML = `<td>${u.username}</td><td>${u.email||''}</td><td>${u.balances.EUR||0}</td><td>${u.balances.BTC||0}</td><td>${u.balances.ETH||0}</td><td>${u.balances.USDT||0}</td><td>${u.banned? 'Yes':'No'}</td><td><button class="ban-btn" data-user="${u.username}">${u.banned? 'Unban':'Ban'}</button> <button class="set-btn" data-user="${u.username}">Set Balance</button> <button class="exp-btn" data-user="${u.username}">Export</button></td>`;
    tbody.appendChild(tr);
    if(list){ const li=document.createElement('div'); li.textContent = u.username; li.dataset.user = u.username; li.addEventListener('click', ()=>{ loadAdminChat(u.username); }); list.appendChild(li); }
  });
  document.querySelectorAll('.ban-btn').forEach(b=>b.addEventListener('click', async ()=>{ const u=b.dataset.user; const ban = b.textContent.trim().toLowerCase()==='ban'; await fetch('/api/admin/ban',{ method:'POST', headers:{ 'Content-Type':'application/json','Authorization':'Bearer '+localStorage.getItem('token') }, body: JSON.stringify({ username:u, ban: ban }) }); fetchUsers(); }));
  document.querySelectorAll('.set-btn').forEach(b=>b.addEventListener('click', async ()=>{ const u=b.dataset.user; const cur=prompt('Currency (EUR,BTC,ETH,USDT):','EUR'); if(!cur) return; const v=parseFloat(prompt('Amount:','0')); if(isNaN(v)) return; await fetch('/api/admin/set-balance',{ method:'POST', headers:{ 'Content-Type':'application/json','Authorization':'Bearer '+localStorage.getItem('token') }, body: JSON.stringify({ username:u, currency:cur, amount: v }) }); fetchUsers(); }));
  document.querySelectorAll('.exp-btn').forEach(b=>b.addEventListener('click', ()=>{ const u=b.dataset.user; window.open('/api/transactions/export?username='+encodeURIComponent(u), '_blank'); }));
}
...
// auto navigate on load (if token present)
if(location.pathname.endsWith('dashboard.html') || location.pathname.endsWith('admin.html')){
  if(!localStorage.getItem('token')) location.href='/index.html';
  else{ if(!socket) connectSocket(); if(location.pathname.endsWith('dashboard.html')) initDashboard(); if(location.pathname.endsWith('admin.html')) initAdmin(); }
}